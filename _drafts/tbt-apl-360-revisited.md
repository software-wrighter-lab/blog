...
You used it from an [IBM 2741](https://en.wikipedia.org/wiki/IBM_2741), a Selectric typewriter wired to a phone line, fitted with the APL typeball so that the keys produced `⍳` and `⍴` and `⌈` instead of the usual characters. The session was a piece of paper. APL\360 indented its prompt six spaces; you typed on the same line; the answer came back flush left. That layout is why an APL transcript is readable at a glance forty years later --- input is indented, output is not --- and it is why sw-apl prints exactly that way.
...
**A homemade APL terminal.** Around 1980, before the PC, I was running an APL on a [TRS-80](https://en.wikipedia.org/wiki/TRS-80) --- one that loaded from a cassette tape, a subset rather than the whole language, and an educational thing rather than a professional one --- and set about giving it the rest of the experience. First the glyphs: I burned an EEPROM with an APL character set and wired a toggle switch to the front of the machine to select between it and the supplied character generator ROM, so the screen could speak APL or English on demand. Then the paper: a used IBM 5471 printer-keyboard, the Selectric-based console from an [IBM System/3](https://en.wikipedia.org/wiki/IBM_System/3), wired to the parallel port on the machine's Expansion Interface --- the box the monitor sat on --- through an interface I built out of TTL chips, so programs could be printed the way APL was meant to be read.

Which is where it got interesting. A good many APL glyphs are not single characters at all but overstrikes: quote-quad is a `⎕`, a backspace, and a `'`, struck in the same place. The obvious driver prints the base character, sends a backspace, and prints the overlay --- and on a used Selectric the backspace did not always take. When it missed, the overlay landed one column to the right and everything after it was wrong. Spaces, on the other hand, never missed.

So I wrote the print driver in assembler and gave it two line buffers instead of one: base characters in the first, overlays in the second, each sitting in its own column. The first pass printed the base line and ended with a carriage return and no line feed, putting the head back at column one of the same line. The second pass then walked that line forward, spacing over every column with nothing to add and striking the ones that had an overlay. Every movement on the page was a space or a print --- not one backspace anywhere --- and the glyphs came out right.
...
Then it went on a desk. The [IBM 5100](https://en.wikipedia.org/wiki/IBM_5100) of 1975 was a fifty-pound "portable" with a five-inch screen showing sixteen lines of sixty-four characters, a tape cartridge for storage, APL glyphs printed on the keys, and a switch on the front to choose between APL and BASIC. It came six years before the 5150, the one IBM called the PC. What makes it remarkable is how the language got in there: rather than write a new interpreter for a small machine, IBM put a processor card in it that emulated a System/370, and ran the mainframe APL interpreter --- APLSV, barely modified --- on that. I serviced the 5100 series as an IBM customer engineer, and the part of that story you learn with the covers off is that the emulation was deliberately slowed --- fast enough to be a fine desktop APL, not fast enough to sell against the machines in the datacenter. The [5110](https://en.wikipedia.org/wiki/IBM_5110) added a diskette drive in 1978, and the 5120 put the whole thing on a desk in 1980.
