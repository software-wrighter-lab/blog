...
Then it went on a desk. I used APL on the IBM 5100 and its successors, the 5110 and the 5120 --- the machines that came six years before the 5150, the one IBM called the PC. The 5100 in 1975 was a fifty-pound "portable" with a five-inch screen showing sixteen lines of sixty-four characters, a tape cartridge for storage, APL glyphs printed on the keys, and a switch on the front to choose between APL and BASIC. What makes it remarkable is how IBM got the language in there: rather than writing a new interpreter for a small machine, they put microcode in it that emulated a System/360 well enough to run the mainframe APL interpreter itself. The whole language came along --- the same primitives, the same `)SAVE` and `)LOAD`, the same workspace --- on a machine you could carry, badly. The 5110 added a diskette drive in 1978 and the 5120 put it on a desk in 1980.
