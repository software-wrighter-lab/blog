...
| **Campus Docent** | [the campus](https://software-wrighter-lab.github.io/sw-campus/#/) --- ask the easel in any lobby |
...
A synthetic domain is the right place to learn the mechanism and the wrong place to stop, so the microscope's first real-world model is a **docent** for the [Software Wrighter Research Campus](https://software-wrighter-lab.github.io/sw-campus/#/) --- the isometric map of the public work from [last week's post](/2026/09/12/software-wrighter-research-campus/). Every lobby has an easel. The easel shows a featured exhibit, and underneath it you can ask the docent where something is.

The design decision that makes a 7,000-parameter model useful here: **the model predicts meaning; the campus catalog supplies the facts.** A question like *"where can I try array programming?"* goes through hashed word features, a router, and a handful of tiny experts, and comes out as an intent --- navigate, explain, recommend, or *not something I know about* --- and a distribution over destinations. The catalog turns that into a title, a summary, a breadcrumb, and an exact URL. The weights never hold a description that can go stale, and the docent cannot invent an exhibit that does not exist, because it never generates text at all. Templated answers are a feature: they let the demonstration prove what the mixture itself learned.

The first version knows three destinations chosen to be semantically far apart --- the IBM 1130, APL, and the RCA 1802 --- so the experts have something distinct to specialize on: a minicomputer, a programming language, a microprocessor. Ambiguous questions are in the training set on purpose. *"Show me an old programming environment associated with IBM systems"* lights up both a languages expert and a history expert, which is top-2 routing you can understand without reading a paper. Out-of-domain questions are there too, so the docent can say plainly that it only knows the campus.

It trains in the browser. The model runs in a Web Worker so the campus keeps drawing while it learns; a few hundred examples take well under a second, and you can watch the accuracy climb epoch by epoch and the expert utilization bars settle. Beside the answer there is a *Why this?* button, which is the microscope in miniature: which experts the router chose, with what weight, on which words. And from there, one link into the full moe-microscope.

The best experiment in the repository is the one the campus hands it for free. The 1442 card reader and its radio demonstration --- the card reader that could play music on a nearby transistor radio --- are not on the campus yet. When they arrive, the catalog knows immediately, and the model does not. Ask the old model where to hear the card reader play music and it answers, reasonably, with the IBM 1130; the catalog notices nothing specific matched, and says so instead of inventing an exhibit. The easel shows *campus revision B, docent trained on A*. Then you add a handful of questions about card readers and press retrain, and watch what the experts do: whether the IBM-history expert absorbs the new material or a new specialization emerges, and whether learning the 1442 made the docent any worse at recognizing APL. Both snapshots are kept --- catalog, training set, weights, metrics --- so the comparison is a table rather than an impression.

That is a thing people hear about constantly and never get to see at understandable scale: *the world changed, the application's data knows it, the model doesn't --- now watch the model learn the change.* It turns a campus chatbot into a small, honest continual-learning experiment, with catastrophic forgetting as a number in a results row.
